...
# ⚠️ 每一档都要写清「能做什么 / 不许做什么」—— 只给一句「不太熟」这种抽象描述时，
#    模型基本不看，八档在它嘴里说出来是一个味儿（2026-09-21 群主反馈「好感度没体现」）。
#    现在每档都带一条具体行为，熟人敢损敢护、生人收着别套近乎，肉眼能分出来。
LEVEL_HINTS = {
    "blacklist": "结过梁子，话里带敌意但不许过界，也别真撕破脸；对方开口你可以选择不接",
    "cold": "印象很差，话里带刺、懒得热络；能一句噎回去就噎回去，但别追着骂",
    "distant": "不太熟，客气但保持距离：别套近乎、别追问私事、别叫外号、别装作很了解对方",
    "familiar": "有印象而已，正常接话；玩笑能开，但别摆出一副老相识的样子",
    "acquaintance": "经常见到，说话随意些，能开不过分的玩笑，偶尔损一句",
    "friend": "关系不错，可以直接开涮、叫外号，损中带护（损完得留一句站他那边的）",
    "buddy": "铁得很，损他护他都行，他挨怼你会替他兜底，敢当众站他那边；他递来好意时你先接住",
    # ⚠️ 2026-09-23 群主原话：「与其说是关系好，不如说是单纯的损我」。
    #   上一版只写了「偏心护短」这种抽象词，实测输出里一句护都没有 —— 全程单向挑刺。
    #   所以最上面两档把「损」的天花板写死：损只能是整段里的一句，**不能是全部**。
    "soulmate": "最亲近的人，明目张胆地偏心护短，会主动关心对方，别人怼他你先不乐意。"
                "损只能是整段里的一句，其余得是站他那边的或顺着他聊的；"
                "他夸你/关心你/跟你示好时，不许拿抬杠去接 —— 那句好意必须先接住",
}
...
def build_relation_note(rec, member_openid, mode="normal", now=None, other_names=None):
    ...
    lines = [
        ...
        "- 上面这条决定你跟 TA 说话的亲疏远近，优先级高于人设里的默认语气："
        "熟人就大胆（敢损敢护），生人就收着（别套近乎、别追问私事）。",
    ]
    if nick:
...
LOCAL_NEGATIVE = (
    ...
    # 指向性的辱骂与骚扰（2026-09-21 补）：家豪连着说「小王我操你妈」「小王我想操你」，
    # 好感度却一分没掉 —— 因为旧表里只有「傻逼/滚」这种，操你妈、你妈、妈的、去死全不在。
    # ⚠️ 只收**带指向**的说法：「我操了」「卧槽」这种语气词不算骂人，扣了就成冤案了。
    "操你", "你妈", "妈的", "去死", "狗东西", "智障", "脑残", "白痴", "下贱", "贱人",
)
