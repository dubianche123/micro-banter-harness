...
| 回归测试 | **403 项** | 改名 / 主名守卫 / 重名拦截 / 认主与防抢答 / 名字刷新 / 压缩署名 / 起名锁 / 提示词顺序 / 提示词不许写死例句 / 语气收束与插嘴自由接茬 / 人物名册注入 / 本地指令不进长期记忆 / 查好感要有查询意图才触发 / 亲密度影响插嘴概率 / 指向性辱骂要扣好感 / 好感度分档与跨档播报 / 模式切换的交情门槛 / 长期记忆不许当万能梗（旧事不自动续杯） / 被动回复时效 / 私聊改名的指路 / 回复里不许冒出 openid / 群昵称的配对与兜底 / 改名节流 / 结构性故障隔离各有专测；除「故障转移」那组要真连供应商外，其余全离线、不需要 Key |
