...
**把机器人拉进群、开启之后，它看到的是群里全部消息，而不是只有 @ 它的那几条。** 这是本项目的前提：它得靠上下文判断该不该接话、记不记得住事，只收到 @ 的那几条等于瞎了。
...
平台侧必须给它开**全量群消息权限**（在 QQ 开放平台的消息订阅里配）。代码这边也补了一层事件解析 —— `botpy` 默认只分派 @ 消息，`bot.py` 顶部把 `GROUP_MESSAGE_CREATE` 也分派进了主流程。只开 @ 权限的话，它照样能回你，但「主动插嘴」「记得住上下文」这些全都不成立，而且**不报错**，只是静默地变傻。
...
| 现象 | 多半是 |
|:--|:--|
| 群里发消息它完全没反应 | ①先看**触发词**：`BOT_NAMES` 没配、平台昵称也拿不到，它只认「机器人」这个通用词；②平台侧没给全量消息权限；③群没加进沙箱群列表 |
| 它只会回 @，不会主动插嘴、也记不住上下文 | 平台侧只开了 @ 权限，没开**全量群消息**权限 —— 它收不到全部消息，就没法判断该不该接话。见「五分钟跑起来」第 1 步那张表 |
