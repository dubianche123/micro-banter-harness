...
或者干脆在 QQ 那边把机器人昵称改掉 —— 进程一起来就知道，不用改代码、不用改配置。

留空则**只认平台昵称**；平台昵称也拿不到时才退回通用词「机器人」。别名是真实需求：有人直接喊名字，有人喊「@名字」，指的都是它，所以支持逗号分隔多个。

> ⚠️ 别把**触发词**和**称呼**混了：触发词是「机器人在你们群里叫什么」（本节），称呼是「它怎么称呼群友 / 群主」（第 2 步的认主、`relations.py`）。两件事都靠配置、都不写死在代码里，但**触发词不配好，后面所有功能都触发不了**。

#### 顺带说清：它只适配「收全量消息」那种机器人

QQ 机器人接消息有两种做法，本项目**只适配第二种**：

| 模式 | 能收到什么 | 适不适合本项目 |
|:--|:--|:--|
| 只收 @ 消息 | 只有被 @ 的那几条 | ❌ 不够用。它要判断该不该接话、要理解上下文，只收 @ 等于瞎了 |
| 收全量群消息 | 群里所有消息 | ✅ **本项目按这个写的**，也请按这个配 |

平台侧必须给它开**全量群消息权限**。代码这边也补了一层事件解析 —— `botpy` 默认只分派 @ 消息，`bot.py` 顶部把 `GROUP_MESSAGE_CREATE` 也分派进了主流程。只开 @ 权限的话，它照样能回你，但「主动插嘴」「记得住上下文」这些全都不成立，而且**不报错**，只是静默地变傻。
...
- **消息权限按第 1 步那种「收全量群消息」来开**，不能只开 @ 权限 —— 理由和第 1 步里那张表一样。
...
| 注入之前 | 过一遍 `refresh_names`：旧字面换成当事人当前的称呼；撤销过称呼的退成「（未留名·XXXX）」 |
...
| 问题 | 解法 | 在哪 |
|:--|:--|:--|
| **触发词（它在群里叫什么）** ← 最优先 | `.env` 的 `BOT_NAMES`（可多别名）+ 平台昵称，运行期解析；代码里不留默认人名 | `naming.py` |
...
| 旧名字残留在记忆里 | 台账 + 注入前刷新，模型只看得到当前那一版 | `relations.RenameLedger` |
...
| 称呼审核准确率 | 4.7 **7/7**；4.5-air 漏谐音且误杀正常昵称；3.5-flash-lite **8/8** | 2 个敏感称呼 + 6 个正常昵称的对照 |
| 压缩吞吐 | 4.5-air 吞下 260 条 / 4732 字；4.7 同一份 1301 contentFilter | 压缩用弱档的实证 |
| 成本闸门 | 全局 ≤3000 次/天；每群令牌桶 8 个、每 5 秒回补 1 个 | 疯刷时压到平均约 12 次/分钟 |
| 回归测试 | **177 项** | 改名 / 主名守卫 / 重名拦截 / 认主与防抢答 / 名字刷新 / 提示词顺序 各有专测；除「故障转移」那组要真连供应商外，其余全离线、不需要 Key |
...
| 不做 | 原因 |
|:--|:--|
| 不做 per-user 配额 | 群聊里人人是平等的群友，按人限流反而奇怪；改成群级令牌桶 + 全局日预算，账单可控就够了 |
...
| 不写固定的金句示例 | 提示词里堆例句，模型会当模板抄（实测 5 条输出 5 条同一个开头），所以只描述想要的效果 |
...
| 现象 | 多半是 |
|:--|:--|
| 群里发消息它完全没反应 | ①先看**触发词**：`BOT_NAMES` 没配、且平台昵称也拿不到，它只认「机器人」这个通用词；②平台侧没给全量消息权限；③群没加进沙箱群列表 |
| 它只会回 @，不会主动插嘴、也记不住上下文 | 平台侧只开了 @ 权限，没开**全量群消息**权限 —— 见「五分钟跑起来」第 1 步那张表 |
| 喊它没反应，但 @ 它有反应 | 触发词和你们实际喊的名字对不上：`BOT_NAMES` 里补上，或直接在 QQ 那边改机器人昵称 |
...
| 起名被拒 | 两道闸：本地词表（职务/长辈称谓 + `sensitive_nicks.txt`）+ 模型审核；审核服务挂了会放行，本地词表仍在兜底 |
| 改名了它还用旧名字叫 | 现在不该出现了：注入前会统一刷新，改名时也会清历史文本。若仍有，看一眼 `state.json` 的 `renames` 里有没有登记到那个旧名 |
...
| 想干的事 | 怎么改 |
|:--|:--|
| 搬到另一个群 | 改**触发词**就行：`BOT_NAMES` 改成你们群的叫法（或在 QQ 那边改昵称），再让它自己认称呼；代码一行不用动 |
| 改它的触发词 / 叫法 | `BOT_NAMES`（`.env`，多别名逗号分隔），或在 QQ 那边改昵称 —— 见「五分钟跑起来」第 1 步 |
| 换主供应商 | `AI_PROVIDER`（两家都是 OpenAI 兼容，代码共用） |
| 多把 Key 轮询 | 在同目录建 `api_keys_gemini.txt` / `api_keys_zhipu.txt`，一行一把，`#` 开头是注释 |
| 调插嘴频率 | `BANTER_PROBABILITY`（默认 0.08）、`OWNER_MENTION_PROBABILITY`（默认 0.25）、`BANTER_COOLDOWN_SECONDS` |
