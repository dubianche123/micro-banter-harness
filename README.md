...
| 回归测试 | **222 项** | 改名 / 主名守卫 / 重名拦截 / 认主与防抢答 / 名字刷新 / 提示词顺序 / 被动回复时效 / 私聊改名的指路 / 回复里不许冒出 openid / 群昵称兜底各有专测；除「故障转移」那组要真连供应商外，其余全离线、不需要 Key |
...
| 不在群里说机器编号 | openid、后四位、群号只出现在日志里。它们一旦进了 prompt（群聊背景、关系档案、@ 的兜底文案，甚至改名的确认语），模型就会当成人名原样复读，群里看见的就是一串读不懂的乱码 |
...
| 不查平台的群成员接口拿昵称 | 接口存在但要单独申请权限（实测 `400 11253 应用无接口访问权限`），为了一个显示名多一道审批不值。改成从正文里 @ 人时留下的**明文昵称**学，而且**只在一条消息只 @ 了一个人时**才记 —— 多人 @ 时无法判断哪个名字对应哪个人，挂错人比不知道更糟 |
